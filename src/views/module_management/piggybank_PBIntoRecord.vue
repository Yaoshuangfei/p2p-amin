<template>
	<section>
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" border style="width: 100%;min-width: 1080px;">
			...
			<el-table-column prop="turnInOrOutMoney"  label="转入金额">
			</el-table-column>
		...
		methods: {
			getlist(){
				...
				if(this.value !== '' && this.value[0] !== null){
					...
					url = baseUrl+'/pro/proCoinAdminManage/userWalletTurnInOrOutRecord?type=0&currentPage='+_this.page+'&token=b312595f-c654-43f5-9cc9-3af27a1130e2'+'&startDate='+startDate+'&endDate='+endDate
				}else{
					url = baseUrl+'/pro/proCoinAdminManage/userWalletTurnInOrOutRecord?type=0&currentPage='+_this.page+'&token=b312595f-c654-43f5-9cc9-3af27a1130e2'
				}
				$.ajax({
                    type:'GET',
                    dataType:'json',
