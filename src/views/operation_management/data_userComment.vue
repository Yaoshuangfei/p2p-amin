 <template>
	<section>
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;min-width: 1080px;">
			...
			<el-table-column prop="platform"  label="评论途径">
			</el-table-column>
			<el-table-column prop="content"  label="评论内容">
			</el-table-column>
			<el-table-column prop="insertTime"  label="评论时间">
			</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			getlist(){
				const _this = this
                var url = baseUrl+"/admin/opComment/userCommentMessageList?token="+_this.token+'&currentPage='+_this.page
                $.ajax({
                    type:'GET',
                    dataType:'json',
                    url:url,
